...
export default function CreateQuotationScreen() {
  ...
  const generateNewQuotationNumber = () => {
    ...
    setQuotationNumber(`${prefix}-${year}${month}-${random}`);
  };
  ...
  const createQuotation = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const quotationId = createdQuotation[0]?.id;
      ...
      Alert.alert('Error', `Failed to create quotation: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };

  const goToQuotationDetail = () => {
    setShowSuccessModal(false);
    if (createdQuotationId) {
      router.push({
        pathname: '/quotation-detail',
        params: { id: createdQuotationId }
      });
    } else {
          ...
          <View style={styles.calendarModalContent}>
            <View style={styles.calendarHeader}>
              <Text style={styles.calendarTitle}>
                Select {calendarMode === 'issue' ? 'Issue Date' : 'Valid Until'}
              </Text>
      ...
      <View style={styles.successModalOverlay}>
        <View style={styles.successModalContent}>
          ...
          <TouchableOpacity 
            ...
            onPress={goToQuotationDetail}
          >
            <Text style={styles.successButtonText}>View Quotation</Text>
          </TouchableOpacity>
          ...
          >
            <Text style={styles.successSecondaryText}>Go to Quotations</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
  ...
  const renderQuotationDetails = () => (
    <View style={styles.section}>
      <View style={styles.sectionHeader}>
        <Icon name="file-contract" size={20} color="#C3B1E1" />
  ...
  const renderNotesSection = () => (
    <View style={styles.section}>
      ...
      <View style={[styles.inputContainer, { marginTop: 16 }]}>
        <Icon name="file-contract" size={16} color="#C3B1E1" style={styles.inputIcon} />
        <TextInput
          style={[styles.input, { height: 80, textAlignVertical: 'top' }]}
        ...
        {loading ? (
          ...
          <>
            ...
            <Text style={[
              ...
              {isConnected ? 'Create Quotation' : 'Offline'}
            </Text>
          </>
        )}
      </TouchableOpacity>
